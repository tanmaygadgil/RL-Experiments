...
while True:  # Run until solved
    ...
    episode_count += 1
    print(f"running reward: {running_reward} at episode {episode_count} with episode reward: {episode_reward} and epsilon:{epsilon}")
...
for i in range(len(episode_reward_history)):
    try:
        moving_ave.append(sum(episode_reward_history[i - 10:i]) / 10)
    except:
        moving_ave.append(0)

plt.plot(range(len(episode_reward_history)), episode_reward_history)
plt.plot(range(len(episode_reward_history)), moving_ave)
plt.show()