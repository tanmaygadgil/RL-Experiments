...
class Model(keras.Model):
    def __init__(self, action_size):
        ...
        self.adv_vals = keras.layers.Dense(action_size, activation = 'linear')
        self.v_vals = keras.layers.Dense(1, activation = "linear")
        
    def call(self, input):
        x = self.layer1(input)
        x = self.layer2(x)
        # x = self.layer3(x)
        adv_vals = self.adv_vals(x)
        v_vals = self.v_vals(x)
        
        q = v_vals + (adv_vals - tf.reduce_mean(adv_vals, axis = 1, keepdims=True))
        return q
        ...
        for episode in range(EPISODES):
            ...
            while True:
                ...
                if frames % UPDATE_AFTER_ACTIONS == 0:    
                    ...
                    q_vals_new_max = np.max(q_vals_new, axis = 1)
                    
                    # r = np.arange(len(q_vals_new))
                    
                    q_target = rewards + GAMMA * q_vals_new_max * (1 - dones)
