...
for epoch in range(epochs):
    ...
    for t in range(steps_per_epoch):
        if render:
            env.render()
        if global_episodes % RENDER_SAMPLING_INTERVAL == 0:
        ...
        if terminal or (t == steps_per_epoch - 1):
            ...
            num_episodes += 1
            print(f"episode_return-{global_episodes}: {episode_return}")
            if global_episodes % RENDER_SAMPLING_INTERVAL == 0:
                    file_str = f"{global_episodes}-{episode_return}.gif"
